...
import pickle  # to store all data generated by model for future analysis
import time  # for timestamping data files
...
with open(str(x) + '_seasons.csv', 'wb') as outfile:
    ...
    outcsv.writerow(['season', 'population', 'mean', 'std', 'sem'])
    ...
    for i in range(y):
        ...
        print len(manyruns[1]), len(manyruns[2])
    for season, counts in enumerate(manyruns):
        ...
        outcsv.writerow([season, 'susceptible', s_mean, s_std, s_stderr])
        outcsv.writerow([season, 'resistant', r_mean, r_std, r_stderr])

# dump entire model dataset in case of later analysis requirements
timestr = time.strftime("%Y%m%d-%H%M%S")
pickle.dump(manyruns, open(str(x) + "runs." + timestr + ".data", "wb"))
